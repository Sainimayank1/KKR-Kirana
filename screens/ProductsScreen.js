      ...
      <ScrollView className="flex flex-1 ">
        {
          loading ?
            ...
            data.length > 0 ?
              data.map((item, key) => {
                ...
                return <TouchableOpacity key={key} className="border-b-2 border-gray-200 py-2" onPress={() => navigate.push("Product Screen", { productDetail: item })}>
                  ...
                  <View className="flex flex-row w-full px-4 py-2">
                    {/* Left section */}
                    <View>
                      <Image source={{ uri: item.uri }} style={{ height: hp(15), width: wp(20), objectFit: "contain" }}></Image>
                    </View>

                    {/* RIght section */}
                    <View className="px-4">
                      {/* Product name*/}
                      <Text className="text-lg font-bold flex-wrap">{item.name.length > 25 ? item.name.slice(0,25)+"..." : item.name}</Text>

                      {/* Key feature*/}
                      <Text className="text-gray-500 flex-wrap">{item.keyFeature.length > 25 ? item.keyFeature.slice(0,35)+"..." : item.keyFeature}</Text>
              ...
              :
              <View style={{width:wp(100),height:hp(90)}} className="flex items-center justify-center"><Text>No item</Text></View>
        }
